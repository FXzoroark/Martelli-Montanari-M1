...
rule_test(X, R) :- echo('Test \''), echo(X), echo('\' : '), rule_test(regles(X, R)), echo('\n').
rule_tests(R) :-
    rule_test(X ?= Y, R),
    rule_test(X ?= a, R),
...
%unifie([f(X,Y) ?= f(g(Z), h(a)), Z ?= f(Y)], choix_premier) % True
%unifie([f(X,Y) ?= f(g(Z), h(a)), Z ?= f(X)], choix_premier) % False


%choix_eq_1([f(X)?=f(Y), X?=f(X), X?=Y], []).